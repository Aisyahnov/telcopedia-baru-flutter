    ...
    <div class="row g-0 chat-container border shadow-sm">
        ...
        <div class="col-md-3 chat-sidebar d-none d-md-flex">
            <div class="p-3 border-bottom bg-light d-flex align-items-center">
                <h5 class="fw-bold mb-0">Pesan Saya</h5>
                <span class="badge bg-danger ms-2 rounded-pill shadow-sm" style="font-size: 0.6rem;">Life</span>
            </div>
            <div class="chat-list">
                @forelse($chats as $c)
                    @php
                    ...
                    @endphp
                    <a href="{{ route('chat.room', $c->id) }}" class="chat-item {{ isset($chat) && $chat->id == $c->id ? 'active' : '' }}">
                        <div class="d-flex align-items-center">
                            <div class="bg-light text-danger rounded-circle d-flex align-items-center justify-content-center me-3 border" style="width: 45px; height: 45px;">
                                <i class="fa fa-user"></i>
                            </div>
                            <div class="overflow-hidden">
                                <h6 class="mb-0 fw-bold text-truncate" style="font-size: 0.9rem;">{{ $opponent->name ?? 'Student' }}</h6>
                                <p class="mb-0 small text-muted text-truncate" style="font-size: 0.75rem;">{{ $lastMsg ? $lastMsg->message : 'Mulai chat...' }}</p>
                            </div>
                        </div>
                    </a>
                @empty
                    <div class="text-center p-5 text-muted small">Kotak masuk kosong.</div>
                @endforelse
            </div>
        </div>
            ...
            @if(isset($chat))
                ...
                <div class="chat-header d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="bg-maroon text-white rounded-circle d-flex align-items-center justify-content-center me-3" style="width: 42px; height: 42px; background: #9F1521;">
                            <i class="fa fa-user"></i>
                        </div>
                        <div>
                            <h6 class="mb-0 fw-bold">{{ $opponent->name ?? 'User' }}</h6>
                            <span class="text-success small" style="font-size: 0.7rem;"><i class="fa fa-circle me-1" style="font-size: 6px;"></i> Aktif Sekarang</span>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="bubble-wrapper {{ $isMe ? 'is-me' : 'is-other' }}" id="msg-wrapper-{{ $msg->id }}">
                            ...
                            @if($isMe)
                                <div class="bubble-actions">
                                    <span class="action-link" onclick="enterEditMode({{ $msg->id }}, '{{ addslashes($msg->message) }}')">Ubah</span>
                                    <span class="action-link action-delete" onclick="deleteMessage({{ $msg->id }})">Hapus</span>
                                </div>
                            @endif
                        </div>
                    @endforeach
        ...
        try {
            ...
            if (data.messages && data.messages.length > 0) {
                data.messages.forEach(msg => {
                    ...
                    let actionsHtml = isMe ? `<div class="bubble-actions"><span class="action-link" onclick="enterEditMode(${msg.id}, '${msg.message.replace(/'/g, "\\'")}')">Ubah</span><span class="action-link action-delete" onclick="deleteMessage(${msg.id})">Hapus</span></div>` : '';

                    wrapper.innerHTML = `<div class="bubble"><span id="msg-text-${msg.id}">${msg.message}</span><small class="bubble-time">${time}</small></div>${actionsHtml}`;
